...
const getPredictionAnalysis = () => {
  if (!props.wordsDetails?.length) return '学习数据不足，暂无预测分析'
  
  return 'AI预测如果你保持当前学习频率，预计在14天内词汇量将提升32%，词汇反应速度提升18%。建议增加学习频率以达到更好效果。'
}

const initPredictionChart = () => {
  if (!predictionChart.value || !props.wordsDetails?.length) return
