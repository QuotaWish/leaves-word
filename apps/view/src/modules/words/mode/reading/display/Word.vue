...
import type { ReadingPrepareWord, IReadingWordItem } from '~/modules/words/mode/reading'
import { type EnglishWordData, globalPreference, useTargetData, useWordSound } from '~/modules/words'

const props = defineProps<{
  prepare: ReadingPrepareWord
}>()
...
const data = reactive<{
  current: IReadingWordItem | null
  next: IReadingWordItem | null
}>({
  current: null,
  next: null,
})
 ...
 */
async function slideCard(direction: 'left' | 'right', nextCardData: IReadingWordItem) {
