...
import type { WordDerived } from '~/composables/api/types';
...
const derived = computed(() => {
  return props.word.content?.derived
})

const options = reactive<{
  open: boolean
  data: WordDerived | undefined
}>({
  open: false,
  data: undefined
})

const handleDerivedInfo = (item: WordDerived) => {
  options.open = true
  options.data = item
}
</script>
...
<template>
  <div v-if="derived" class="WordDerivedDisplayer">
    <div :class="{ single: derived.length === 1 }" v-for="item in derived" :key="item.id"
      class="WordDerivedDisplayer-Item">
      <div class="WordDerivedDisplayer-Item-Decorator">
      ...
      <template v-if="item">
        <p @click="handleDerivedInfo(item)" cursor-pointer class="hover:color-[var(--theme-color-primary)]" flex
          ...
          <span class="type-badge fake-background">{{ transformDerivedType(item.type).name }}</span>
          <span text-sm op-75>
    ...
    <TouchDialog targetAnimation=".WordDetailContent" v-model="options.open">
      <div v-if="options.data" class="derived-dialog">
        <div class="derived-header">
          <span class="type-badge">{{ transformDerivedType(options.data.type).name }}</span>
          <span class="derived-text">{{ transformDerivedType(options.data.type).description }}</span>
        </div>

        <div class="derived-section fake-background">
          <div class="section-title">
            <i class="section-icon" i-carbon-text-annotation></i>
            <span>定义 {{ options.data.content }}</span>
          </div>
          <p class="section-content">{{ (options.data.data as any).definition }}</p>
        </div>
...
.WordDerivedDisplayer {
  ...
  &-Item {
    ...
    &:last-child {
      .WordDerivedDisplayer-Item-Decorator {
        &::before {
          opacity: 0;
        }
      }
