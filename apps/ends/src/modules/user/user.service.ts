...
export class UserService {
  ...
  async findUserByUserPhone(phone: string): Promise<UserEntity | undefined> {
    return this.userRepository
      .createQueryBuilder('user')
      .where({
        phone,
  ...
  async search(query: string): Promise<[UserEntity[], number]> {
    return this.userRepository
      .createQueryBuilder('user')
      .where({
        username: Like(`%${query}%`),
  ...
  async smsLoginReg(phone: string): Promise<void> {
    const exists = await this.userRepository.findOneBy({
      phone,
    })
    if (!isEmpty(exists))
    ...
    await this.entityManager.transaction(async (manager) => {
      ...
      const password = md5(`${randomValue(32)}${salt}`)
      ...
      await manager
        ...
        .relation(UserEntity, 'roles')
        .of(user.id)
        .addAndRemove([2], user.roles)

      await this.menuService.refreshPerms(user.id)
    })
  }

  async platformLoginReg(phone: string): Promise<void> {
    const exists = await this.userRepository.findOneBy({
      phone,
    })
    if (!isEmpty(exists))
